function loadScheduleTemplate() {
	...
	ScheduleTemplate.prototype.placeEvents = function() {
		...
		for(var i = 0; i < this.singleEvents.length; i++) {
			...
			var eventTop = (slotHeight-1)*(start - self.timelineStart)/self.timelineUnitDuration,
				eventHeight = slotHeight*duration/self.timelineUnitDuration;

			this.singleEvents[i].setAttribute('style', 'top: '+(eventTop-2)+'px; height: '+(eventHeight)+'px');
		}
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		this.modalDate.textContent = `${target.getAttribute('data-time')} - ${target.getAttribute('data-date')}`;
		this.modal.setAttribute('data-event', target.getAttribute('data-event'));
		this.modal.setAttribute('data-color', target.getAttribute('data-color'));
		...
		if( mq == 'mobile' ) {
			...
			var HeaderBgScaleY = modalHeight/eventHeight,
				BodyBgScaleX = (modalWidth - eventWidth*2);
			...
			self.modalHeader.setAttribute('style', 'width:'+eventWidth*2+'px');
			//set modalBody left margin
			self.modalBody.setAttribute('style', 'margin-left:'+eventWidth*2+'px');
			...
			self.modalHeaderBg.setAttribute('style', 'height: '+eventHeight+'px; width: '+(eventWidth*2)+'px; transform: scaleY('+HeaderBgScaleY+')');
	...
	ScheduleTemplate.prototype.loadEventContent = function(content) {
		...
		httpRequest.onreadystatechange = function() {
	      ...
	      if (httpRequest.status === 200) {*/
			  self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = /*self.getEventContent(*/content/*httpRequest.responseText)*/;
			  let work_mark_btn = self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].getElementsByClassName('mark_work_btn')[0];
			  let work_modify_btn = self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].getElementsByClassName('alter_work_btn work_modify')[0];
			  let work_delete_btn = self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].getElementsByClassName('alter_work_btn work_delete')[0];
			  if(work_mark_btn !== undefined)
				if (work_mark_btn.classList.contains('done'))
					work_mark_btn.addEventListener('click', (evt) => markWork(self.modal.getAttribute('data-event'), true, true));
				else if (work_mark_btn.classList.contains('todo'))
					work_mark_btn.addEventListener('click', (evt) => markWork(self.modal.getAttribute('data-event'), false, true));
			  if(work_modify_btn !== undefined && work_delete_btn !== undefined) {
				work_delete_btn.addEventListener('click', (evt) => {
				  if (confirm('Êtes-vous sûr ?')) {
					  deleteWork(evt.target.parentElement.parentElement.parentElement);
				  }
				});
				work_modify_btn.addEventListener('click', (evt) => {
					loadPopupRendu(false, evt.target.parentElement.parentElement.parentElement.getAttribute('data-event').substring(6));
				});
			  }
